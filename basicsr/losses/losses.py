...
def ssim_loss(pred, target):
    return 1 - ssim(pred, target, reduction='none')
...
class SSIM(nn.Module):
    """SSIM loss: returns loss_weight * (1 - SSIM).
    
    Args:
        window_size (int): Size of the Gaussian window for SSIM. Default: 11.
        size_average (bool): If True, average SSIM over all pixels/batches. Default: True.
        loss_weight (float): Weight factor for the final loss. Default: 1.0.
        reduction (str): 'mean' or 'none' or 'navg'. Default: 'mean'.
            - 'mean': average SSIM over all pixels and batches.
            - 'none': returns the per-pixel SSIM map (not typical).
            - 'navg': average over spatial dims only, returning (N,)-shaped output.
    """
    def __init__(self, window_size=11, size_average=True, loss_weight=1.0, reduction='mean'):
        ...
        # By default, we create a window for single-channel. 
        # We'll recreate it if we detect a different channel size at runtime.
        self.channel = 1
    ...
    def forward(self, img1, img2):
        ...
        if self.size_average and self.reduction == 'mean':
            ssim_val = _ssim(img1, img2, window, self.window_size, c, 'mean')
        elif not self.size_average and self.reduction == 'navg':
            ssim_val = _ssim(img1, img2, window, self.window_size, c, 'navg')
        else:
            # fallback or custom usage
            ssim_val = _ssim(img1, img2, window, self.window_size, c, self.reduction)

        # Return the SSIM-based loss: 1 - SSIM
